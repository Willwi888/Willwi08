...
interface LyricsTimingProps {
  ...
  onComplete: (timedLyrics: TimedLyric[], duration: number) => void;
  onBack: () => void;
}
...
const LyricsTiming: React.FC<LyricsTimingProps> = ({ lyricsText, audioUrl, backgroundImageUrl, onComplete, onBack }) => {
  ...
  const handleComplete = () => {
    ...
    onComplete(timedLyrics, audioRef.current?.duration || 0);
  };
  ...
  const isCompleteEnabled = useMemo(() => editableLyrics.length > 0 && timestamps.every(t => t !== null), [timestamps, editableLyrics]);
    ...
    <div className="w-full max-w-7xl mx-auto h-[85vh] flex flex-col">
      <div className="flex-grow flex flex-col lg:flex-row gap-8 items-start overflow-hidden">
        ...
        <div className="w-full lg:w-1/3 flex-shrink-0 lg:sticky lg:top-0">
          <h3 className="text-xl font-bold text-yellow-300 mb-4 text-center">專輯封面</h3>
          <img src={backgroundImageUrl} alt="專輯封面" className="w-full aspect-square object-cover rounded-lg shadow-2xl ring-1 ring-yellow-500/30" />
          <div className="mt-4 p-4 bg-black/50 rounded-lg border border-yellow-700/50">
             <h4 className="font-semibold text-yellow-300 mb-2">快捷鍵</h4>
        ...
        <div className="w-full lg:w-2/3 h-full overflow-y-auto custom-scrollbar bg-black/50 backdrop-blur-sm rounded-lg border border-yellow-700/50">
          <table className="w-full text-left text-gray-200">
            <thead className="sticky top-0 bg-gray-900/80 backdrop-blur-sm z-10">
              <tr className="border-b border-yellow-800">
                <th colSpan={2} className="p-4 text-center text-yellow-300 font-semibold">時間</th>
                <th rowSpan={2} className="p-4 align-bottom text-yellow-300 font-semibold">歌詞</th>
              </tr>
              <tr className="border-b border-yellow-800">
            ...
            <tbody>
              {editableLyrics.map((line, index) => {
                ...
                let rowClassName = 'border-b border-yellow-900/50 transition-colors cursor-pointer';
                if (isLineActive) {
                  rowClassName += ' bg-yellow-500/20';
                } else if (isLinePlaying) {
                  rowClassName += ' bg-yellow-500/10';
                } else {
                  rowClassName += ' hover:bg-yellow-800/20';
                }
                  ...
                  >
                    <td className="p-3 text-center w-28 font-mono">
                      <div className="flex items-center justify-center gap-2">
                        <span className="text-yellow-300">{timestamps[index] !== null ? `${timestamps[index]!.toFixed(2)}s` : '---'}</span>
                    ...
                    <td className="p-3 text-lg">
                       <p 
                        ...
                        className="focus:outline-none focus:bg-yellow-900/50 rounded px-2 -mx-2 cursor-text min-h-[1.5rem]"
                      >
                        {line}
                      </p>
                    </td>
      ...
      {/* Bottom Controls */}
      <div className="flex-shrink-0 mt-4 bg-black/70 backdrop-blur-sm rounded-lg p-4 space-y-3 border border-yellow-700/50">
        <audio 
          ref={audioRef} 
          src={audioUrl} 
        ...
        <div className="flex items-center gap-3">
          ...
          <input
              ...
              className="w-full h-1.5 bg-gray-600 rounded-full appearance-none cursor-pointer accent-yellow-400"
          />
        ...
        <div className="flex items-center justify-between">
          ...
          <button 
              ...
              className="px-6 py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-black font-bold rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
